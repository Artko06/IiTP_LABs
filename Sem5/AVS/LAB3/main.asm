...
; сообщить и выйти
        mov        dx,offset v86_msg ; загружаем в dx сообщение об ошиб ке
...
; в защищенный режим
        ...
        mov        al,'A'               ; Начальный символ для вывода (например, 'A')
task_0:
...
; дальний переход на TSS задачи 1
        ...
        cmp        al,'k'               ; Остановиться на символе 'Z'
        jb         task_0               ; Если AL < 'Z', продолжаем цикл
...
; на задачу 1 во всех случаях, кроме первого
        mov        ecx,10000000h        ; небольшая пауза, зависящая от скорости
        loop       $                    ; процессора
        jmp        task_1
