    ...
    public static void MapCarEndpoints(this IEndpointRouteBuilder routes)
    {
        ...
        ILogger<Program> logger) =>
        {
            try
            {
                ...
                if (newCar == null)
                {
                    ...
                    return Results.BadRequest("Invalid car data"); // ← RETURN
                }

                // Проверяем вручную каждое поле
                if (string.IsNullOrEmpty(newCar.Name))
                {
                    ...
                    return Results.BadRequest("Name is required"); // ← RETURN
                }
                ...
                return Results.Created($"/api/Car/{newCar.Id}", newCar); // ← RETURN
            }
            catch (JsonException jsonEx)
            {
                logger.LogError(jsonEx, "JSON deserialization error");
                return Results.BadRequest($"JSON Error: {jsonEx.Message}"); // ← RETURN
            }
            catch (DbUpdateException dbEx)
            {
                logger.LogError(dbEx, "Database error");
                return Results.BadRequest($"Database Error: {dbEx.Message}"); // ← RETURN
            }
            catch (Exception ex)
            {
                logger.LogError(ex, "Unexpected error in car creation");
                return Results.Problem($"Internal Server Error: {ex.Message}"); // ← RETURN
            }
        })
        .WithName("CreateCar")
        .WithOpenApi();

        // ОБНОВЛЕНИЕ автомобиля с возможностью загрузки файла
            ...
            ILogger<Program> logger) =>
        {
            try
            {
                ...
                // Находим существующий автомобиль
                var existingCar = await db.Cars.FindAsync(id);
                if (existingCar == null)
                {
