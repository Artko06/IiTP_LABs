...
public static class CarEndpoints
{
    public static void MapCarEndpoints(this IEndpointRouteBuilder routes)
    {
        ...
        group.MapGet("/categories/{category:regex(^[a-z0-9\\-]+$)}",
            async (IMediator mediator, HybridCache cache, string? category, [FromQuery] int pageNo = 1, [FromQuery] int pageSize = 3) =>
            {
                var cacheKey = $"cars_{category}_{pageNo}_{pageSize}";
                
                var result = await cache.GetOrCreateAsync(
                    cacheKey,
                    async cancel => await mediator.Send(new GetListOfCars(category, pageNo, pageSize)),
        ...
        group.MapGet("/",
            async (IMediator mediator, HybridCache cache, [FromQuery] int pageNo = 1, [FromQuery] int pageSize = 3) =>
            {
                var cacheKey = $"cars_all_{pageNo}_{pageSize}";

                var result = await cache.GetOrCreateAsync(
                    cacheKey,
                    async cancel => await mediator.Send(new GetListOfCars(null, pageNo, pageSize)),
                    options: new HybridCacheEntryOptions
                    {
                        Expiration = TimeSpan.FromMinutes(1),
                        LocalCacheExpiration = TimeSpan.FromSeconds(30)
